...
{
	char c = uartRead(UART_232);
	uartWrite(UART_USB,c);
}
...
void onTecRaised(tec_t* t){
	uartWriteString( UART_232, "Tecla apretada: " );
	uartWriteString( UART_232, t->name );
...
void onTecFalled(tec_t* t){
	uartWriteString( UART_232, "Tecla liberada: " );
	uartWriteString( UART_232, t->name );
	uartWriteString( UART_232, "\r\n" );
